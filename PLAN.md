...
⚠️ **발행 함정 4건** (다음 버전 올릴 때 또 헤매지 말 것):
...
3. **발행이 중간에 끊기면 그 버전 번호는 영구히 못 쓴다.** npm 이 staged 상태로 남겨서
...
| ✅ M4 | 데모 웹 | **nsite 배포 완료** (2026-09-06). 하드코딩 npub 5글 + 아웃박스 덧글 트리(재귀 아님 — 아래) |
...
### M4 결과 (2026-09-06) — nsite 배포 완료

`apps/demo`. 배포처 = `npub1pt4qw…nsite.lol` (NIP-5A, nsyte + NIP-46 번커 서명).
`SimplePool` → `PaidPool` **한 줄**만 다른 클라이언트라는 주장을 코드로 보이는 게 목적.

설계에서 **바뀐 것 1건**: 덧글 트리는 재귀로 안 긁는다. NIP-10이 덧글에 조상의 `p` 태그를
전부 물려주므로 **깊이와 무관하게 루트 저자의 inbox에 떨어진다** → 한 번의 쿼리로 전체 스레드가
온다. 리액션(kind 7)만 예외 — NIP-25는 아무것도 안 물려줘서 리액션은 *그 덧글 저자*의 inbox로
간다. 그래서 레벨당 배치 조회 1회를 따로 돈다(그래도 재귀는 아님).

일부러 **안 긁는** 것도 데모의 일부다. 스펙을 무시하고 큰 릴레이에 뿌리는 클라를 우리가
보완해주면, 소수 릴레이 집중이 정답이 되고 스펙을 지킨 쪽이 "덜 찾는다"고 욕먹는다.

로그인 모델은 없다. 스팸을 **결제로만** 측정한다는 원칙에서 계정 체계는 아무것도 측정하지
않으므로, 덧글은 브라우저에 생성한 일회성 키로 단다. 붙일 게 NWC 하나뿐이라는 것도 같은 주장이다.

**배포 함정 (nsite)**: 같은 블롭을 블로썸 서버마다 다른 MIME으로 서빙한다 —
`blossom.primal.net`은 CSS를 `text/plain`으로 준다. 브라우저는 strict MIME checking으로
그런 스타일시트를 **적용 거부**한다(JS는 타입이 맞아서 기능만 멀쩡 → 무스타일로 조용히 배포됨).
게이트웨이가 어느 서버를 고를지는 우리 통제 밖이라 서버 교체는 확률만 옮긴다 → **CSS를
index.html에 인라인**(`vite.config.ts`). 죽은 서버(`cdn.satellite.earth` 502)도 같이 교체.
...
| ~~V9~~ | ~~float 단일 writer 락~~ | ✅ **완료**. `float/src/lock.ts` — Web Locks + 인프로세스 뮤텍스 폴백, `EcashFloat`이 항상 경유 |
...
| ~~V6~~ | ~~대상 npub의 kind 10002 부작용~~ | ✅ **소멸**. 데모는 전용 키를 쓴다(실계정 아님). 부작용은 의도한 것 자체 — 그 계정 앞으로 온 덧글은 유료 릴레이에만 존재 |
